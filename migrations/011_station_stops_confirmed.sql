-- Dodanie is_confirmed / is_cancelled do station_stops.
-- is_confirmed=true oznacza że pociąg faktycznie potwierdził przejazd przez ten
-- przystanek (pole isConfirmed z OperationStationDto w API PKP).
-- Zastępuje heurystykę czasową z migracji 010.
--
-- Dodanie train_name do schedules (RouteDto.name z API /schedules).

ALTER TABLE station_stops
    ADD COLUMN IF NOT EXISTS is_confirmed BOOLEAN NOT NULL DEFAULT FALSE,
    ADD COLUMN IF NOT EXISTS is_cancelled  BOOLEAN NOT NULL DEFAULT FALSE;

CREATE INDEX IF NOT EXISTS idx_station_stops_confirmed
    ON station_stops (train_op_id, is_confirmed, planned_sequence DESC);

ALTER TABLE schedules
    ADD COLUMN IF NOT EXISTS train_name VARCHAR(200);

-- Przebudowa v_active_delays z poprawnym filtrem is_confirmed.
...
last_visited AS (
    -- Ostatni przystanek potwierdzony przez API (isConfirmed=true).
    -- Dla danych zebranych przed migracją 011 (is_confirmed=false wszędzie)
    -- fallback do heurystyki czasowej z 2h przesunięcia strefy.
    SELECT DISTINCT ON (ss.train_op_id)
        ss.train_op_id,
        st.name           AS station_name,
    ...
    JOIN latest_snapshot ls   ON to_.snapshot_id = ls.id
    LEFT JOIN stations st     ON ss.station_id = st.station_id
    WHERE ss.is_confirmed = TRUE
       OR (
           -- fallback dla starych danych bez is_confirmed
           NOT EXISTS (
               SELECT 1 FROM station_stops ss2
               JOIN train_operations to2 ON ss2.train_op_id = to2.id
               WHERE to2.snapshot_id = ls.id AND ss2.is_confirmed = TRUE
               LIMIT 1
           )
           AND ss.actual_arrival IS NOT NULL
           AND ss.actual_arrival <= ls.fetched_at + INTERVAL '2 hours'
       )
    ORDER BY ss.train_op_id, ss.planned_sequence DESC
),
train_delay AS (
...
SELECT
    ...
    COALESCE(sc.train_name, c.name) AS carrier_name,
