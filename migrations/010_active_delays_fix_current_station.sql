-- Naprawa last_visited_station: API zwraca actual_arrival dla WSZYSTKICH przystanków
-- (włącznie z przyszłymi, wypełniając je planowanymi czasami), więc filtr
-- IS NOT NULL nie wystarczy. Używamy porównania z fetched_at.
--
-- Offset +2h wynika z: parser zapisuje czasy z API (strefa Europe/Warsaw, UTC+2 latem)
-- jako naive datetime → psycopg3 traktuje je jako UTC → rzeczywiste przesunięcie 2h.
-- W zimie (UTC+1) korekta jest o 1h za duża, co może włączyć przystanek do 1h
-- w przyszłości – akceptowalne jako przybliżenie.
