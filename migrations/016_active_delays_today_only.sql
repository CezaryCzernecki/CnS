-- Ograniczenie v_active_delays wyłącznie do pociągów dzisiejszej daty.
--
-- Zmiana względem migracji 015:
--   CURRENT_DATE - INTERVAL '1 day'  →  CURRENT_DATE
--
-- Motywacja: pociągi z operating_date=wczoraj pojawiały się w widoku
-- jako artefakty starych snapshotów mimo zakończonego biegu. Adnotacja
-- "data wyruszenia" okazała się myląca — użytkownicy nie chcą wczorajszych
-- danych w tablicy aktywnych opóźnień.
-- Pociągi nocne startujące przed północą mają operating_date=dzisiaj
-- (kolektor pobiera rozkłady z zakresu yesterday–today i dopasowuje).
--
-- Migracja jest idempotentna i self-contained: dodaje brakujące kolumny
-- (z migracji 011) jeśli jeszcze nie istnieją, co pozwala ją zastosować
-- na bazie bez historii migracji 011–015.
...
last_visited AS (
    ...
    ORDER BY ss.train_op_id, ss.actual_arrival DESC
),
train_delay AS (
    SELECT
        ss.train_op_id,
