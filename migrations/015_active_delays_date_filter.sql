-- Ograniczenie v_active_delays do dzisiejszych i wczorajszych pociągów.
--
-- Problem: widok pokazywał pociągi z dowolną operating_date, co mogło
-- powodować wyświetlanie starych danych gdy w snapshocie pojawiły się
-- wpisy z przeszłości.
-- Rozwiązanie: filtr operating_date >= CURRENT_DATE - 1 day zachowuje
-- nocne pociągi startujące wczoraj (te mają operating_date = wczoraj
-- i nadal są w trasie), odcina zaś wszystko starsze.
-- Adnotacja daty wyruszenia dla pociągów wczorajszych jest dodawana
-- po stronie dashboardu (delays/page.tsx).
...
WHERE to_.train_status IN ('P', 'X')
  AND to_.operating_date >= CURRENT_DATE - INTERVAL '1 day'
